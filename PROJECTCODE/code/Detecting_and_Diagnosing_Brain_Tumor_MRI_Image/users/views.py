...
def apply_custom_colormap(image_gray, cmap=plt.get_cmap('seismic')):
    ...
    channels = [cv.LUT(image_gray, color_range[:,i]) for i in range(3)]
    return np.dstack(channels)
from django.conf import settings
import os
from .AlgoProcess.predictTumor import predictTumor
from django.core.files.storage import FileSystemStorage
def procesMyTestImages(request):
    if request.method == 'POST':
        ...
        fs = FileSystemStorage("media/testing/")
        filename = fs.save(myfile.name, myfile)
        fName = filename
        uploaded_file_url = fs.url(filename)
        mriImage = os.path.join(settings.MEDIA_ROOT,'testing', fName)
        res = predictTumor(mriImage)
        if res > 0.5:
            text="Tumor Detected"
            import cv2 as cv
            import numpy as np
            import cmapy
            image = os.path.join(mriImage)   
           
            filename = os.path.join(settings.MEDIA_ROOT, 'testing', 'test.png')
            image_gray = cv.imread(image, cv.IMREAD_GRAYSCALE)
            image_bgr = apply_custom_colormap(image_gray, cmap=plt.get_cmap('bwr'))
            # img_colorized = cv.applyColorMap(filename, cmapy.cmap('viridis'))
            plt.imsave(filename, image_bgr, cmap='jet')
        else:
            text="No Tumor"
        print("Processing File Name:",res)
       
        
        p = fs.url(mriImage)
        filename = fs.url(filename)
        return render(request, "users/testForm.html", {'path': p, "res": text,"pp": filename})
    else:
        return render(request, "users/testForm.html", {})
